...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .btn { font-family: 'Inter', sans-serif; font-weight: 600; font-size: 14px; padding: 9px 16px; border-radius: 4px; border: 1px solid transparent; cursor: pointer; }
        ...
        .stat-strip { display: grid; grid-template-columns: 1.3fr 1fr 1fr; gap: 14px; margin-bottom: 28px; }
        .stat-card { background: var(--paper-raised); border: 1px solid var(--line); border-radius: 8px; padding: 18px 20px; box-shadow: 0 1px 2px rgba(28,26,23,.04), 0 4px 12px rgba(28,26,23,.06); }
        .stat-card .num { font-family: 'Fraunces', serif; font-size: 34px; font-weight: 600; color: var(--moss-deep); line-height: 1; }
        .stat-card .num small { font-family: 'Inter', sans-serif; font-size: 13px; font-weight: 500; color: var(--ink-soft); margin-left: 6px; }
        .stat-card.hero { background: var(--moss-deep); color: #EFEEE7; border: none; display: flex; flex-direction: column; justify-content: space-between; }
        .stat-card.hero .eyebrow { color: #B9C9BE; }
        .stat-card.hero p { font-family: 'Fraunces', serif; font-size: 18px; font-weight: 400; font-style: italic; margin: 0; line-height: 1.4; color: #FBFAF7; }
        ...
        .cat-card .thumb { position: relative; aspect-ratio: 16 / 11; background: var(--moss-tint); overflow: hidden; }
        ...
        .cat-card .name { font-family: 'Fraunces', serif; font-size: 17px; font-weight: 600; line-height: 1.3; }
        .cat-card .status { font-size: 12px; font-weight: 600; }
        
        /* Hai class mới thay thế cho inline style bị báo lỗi */
        .status-active { color: var(--moss); }
        .status-locked { color: var(--rust); }
        ...
        .cat-card .row-actions a.edit-link { color: var(--terra); } 
        .cat-card .row-actions a.danger { color: var(--rust); }
    ...
    <div class="shell">
        ...
        <main class="main">
            <div class="page-head">
                ...
                <div class="actions">
                    <a href="${pageContext.request.contextPath}/admin/categories"><button type="button" class="btn btn-ghost">Tải lại</button></a>
            ...
            <div class="stat-strip">
                ...
                <div class="stat-card">
                    ...
                    <div class="num">${fn:length(cateList)}<small>mục</small></div>
                </div>
                <div class="stat-card">
                        ...
                        <c:forEach items="${cateList}" var="cate" varStatus="STT">
                            ...
                            <div class="cat-card">
                                ...
                                <div class="body">
                                    ...
                                    <!-- Đã dùng class thay cho inline css ở đây -->
                                    <div class="status ${cate.status == 1 ? 'status-active' : 'status-locked'}">
                                        Trạng thái: ${cate.status == 1 ? 'Hoạt động' : 'Khóa'}
                                    </div>
                                    
                                    <div class="row-actions">
                                        <a class="edit-link" href="${pageContext.request.contextPath}/admin/category/edit?id=${cate.categoryId}">Sửa</a>
                                        <a class="danger" href="${pageContext.request.contextPath}/admin/category/delete?id=${cate.categoryId}" onclick="return confirm('Bạn có chắc chắn muốn xóa danh mục này?');">Xóa</a>
                                    </div>
                                </div>
                            </div>
                        </c:forEach>
